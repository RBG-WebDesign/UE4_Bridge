...
export function createOperationsTools(
  ...
  return [
    ...
    },
    {
      ...
      handler: async (params) => {
        const operations = params.operations as Array<{ command: string; params?: Record<string, unknown> }>;
        const results: Array<{ command: string; success: boolean; data?: unknown; error?: string }> = [];
